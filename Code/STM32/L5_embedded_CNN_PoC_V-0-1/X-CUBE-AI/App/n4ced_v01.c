...
#define AI_NET_OBJ_INSTANCE g_n4ced_v01
 
#undef AI_N4CED_V01_MODEL_SIGNATURE
#define AI_N4CED_V01_MODEL_SIGNATURE     "dbd11f49d2f2827ffaa2cd3ff5de195b"
...
#define AI_TOOLS_DATE_TIME   "Mon Feb 28 12:09:38 2022"

#undef AI_TOOLS_COMPILE_TIME
#define AI_TOOLS_COMPILE_TIME    __DATE__ " " __TIME__

#undef AI_N4CED_V01_N_BATCHES
#define AI_N4CED_V01_N_BATCHES         (1)
...
AI_TENSOR_OBJ_DECLARE(
  ...
  17, 0x0,
  AI_SHAPE_INIT(4, 2400, 64, 1, 1), AI_STRIDE_INIT(4, 1, 2400, 153600, 153600),
...
AI_NETWORK_OBJ_DECLARE(
  ...
  AI_BUFFER_INIT(AI_FLAG_NONE,  AI_BUFFER_FORMAT_U8,
    ...
    40896, NULL, NULL),
  AI_TENSOR_LIST_IO_OBJ_INIT(AI_FLAG_NONE, AI_N4CED_V01_IN_NUM, &serving_default_conv2d_input0_output),
  AI_TENSOR_LIST_IO_OBJ_INIT(AI_FLAG_NONE, AI_N4CED_V01_OUT_NUM, &nl_8_output),
...
/******************************************************************************/
AI_DECLARE_STATIC
ai_bool n4ced_v01_configure_activations(
...
/******************************************************************************/
AI_DECLARE_STATIC
ai_bool n4ced_v01_configure_weights(
  ai_network* net_ctx, const ai_network_params* params)
{
  AI_ASSERT(net_ctx)
  ...
  if (ai_platform_get_weights_map(weights_map, 1, params)) {
    ...
    dense_6_bias_array.data = AI_PTR(weights_map[0] + 280384);
    dense_6_bias_array.data_start = AI_PTR(weights_map[0] + 280384);
    dense_7_weights_array.format |= AI_FMT_FLAG_CONST;
    dense_7_weights_array.data = AI_PTR(weights_map[0] + 280640);
    dense_7_weights_array.data_start = AI_PTR(weights_map[0] + 280640);
    dense_7_bias_array.format |= AI_FMT_FLAG_CONST;
    dense_7_bias_array.data = AI_PTR(weights_map[0] + 282432);
    dense_7_bias_array.data_start = AI_PTR(weights_map[0] + 282432);
    return true;
  }
  ...
  ai_handle network, ai_network_report* report)
{
  ...
  {
    ai_network_report r = {
      .model_name        = AI_N4CED_V01_MODEL_NAME,
      .model_signature   = AI_N4CED_V01_MODEL_SIGNATURE,
...
AI_API_ENTRY
ai_bool ai_n4ced_v01_get_report(
  ai_handle network, ai_network_report* report)
{
  ai_network* net_ctx = AI_NETWORK_ACQUIRE_CTX(network);
...
AI_API_ENTRY
ai_error ai_n4ced_v01_get_error(ai_handle network)
{
  return ai_platform_network_get_error(network);
}

AI_API_ENTRY
ai_error ai_n4ced_v01_create(
...
AI_API_ENTRY
ai_error ai_n4ced_v01_create_and_init(
  ai_handle* network, const ai_handle activations[], const ai_handle weights[])
{
    ai_error err;
    ai_network_params params;

    err = ai_n4ced_v01_create(network, AI_N4CED_V01_DATA_CONFIG);
    if (err.type != AI_ERROR_NONE)
        return err;
    if (ai_n4ced_v01_data_params_get(&params) != true) {
        err = ai_n4ced_v01_get_error(*network);
        return err;
    }
#if defined(AI_N4CED_V01_DATA_ACTIVATIONS_COUNT)
    if (activations) {
...
#endif
#if defined(AI_N4CED_V01_DATA_WEIGHTS_COUNT)
    if (weights) {
...
#endif
    if (ai_n4ced_v01_init(*network, &params) != true) {
        err = ai_n4ced_v01_get_error(*network);
    }
    return err;
}
...
AI_API_ENTRY
ai_buffer* ai_n4ced_v01_inputs_get(ai_handle network, ai_u16 *n_buffer)
{
...
AI_API_ENTRY
ai_buffer* ai_n4ced_v01_outputs_get(ai_handle network, ai_u16 *n_buffer)
{
  if (network == AI_HANDLE_NULL) {
    network = (ai_handle)&AI_NET_OBJ_INSTANCE;
    ((ai_network *)network)->magic = AI_MAGIC_CONTEXT_TOKEN;
...
AI_API_ENTRY
ai_handle ai_n4ced_v01_destroy(ai_handle network)
{
  return ai_platform_network_destroy(network);
}

AI_API_ENTRY
ai_bool ai_n4ced_v01_init(
  ai_handle network, const ai_network_params* params)
{
  ...
  ai_bool ok = true;
  ok &= n4ced_v01_configure_weights(net_ctx, params);
  ok &= n4ced_v01_configure_activations(net_ctx, params);
...
AI_API_ENTRY
ai_i32 ai_n4ced_v01_forward(ai_handle network, const ai_buffer* input)
{
